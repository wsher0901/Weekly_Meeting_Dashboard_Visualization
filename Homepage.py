...
a5.page_link("sections/1_Pre_PCR_High_Volume.py", label="Pre PCR High Volume", icon="💉", disabled = st.session_state['Pre PCR (High Vol) status'])
a5.page_link("sections/2_Pre_PCR_CMV.py", label="Pre PCR CMV", icon="🦠", disabled = st.session_state['Pre PCR (CMV) status'])
a5.page_link("sections/3_Pre_PCR_Low_Volume.py", label='Pre PCR Low Volume', icon='🏥',disabled = st.session_state['Pre PCR (Low Vol) status'])
a5.page_link("sections/4_PCR.py", label='PCR', icon="🧪",disabled = st.session_state['PCR status'])
a5.page_link("sections/5_Gel.py", label='Gel', icon="🧫",disabled = st.session_state['Gel status'])
...
if os.path.exists('Archive/'):
    ...
    for ind,d in enumerate(entries,0):
        ...
        if meeting:
            ...
            st.switch_page("sections/1_Pre_PCR_High_Volume.py")
