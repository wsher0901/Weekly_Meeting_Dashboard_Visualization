...
def generate_jobs_bar_chart(df):
    ...
    modified_df = modified_df.sort_values(by=['Machine Type','Date'],ascending=True).reset_index(drop=True)
    modified_df.index += 1
...
def style_total_cell_table(df):
    ...
    modified_df['Machine Type'] = pd.Categorical(modified_df['Machine Type'], categories=['Pacbio Sequel-I','Pacbio Sequel-II','Pacbio Sequel-IIe'],ordered=True)
    modified_df = modified_df.sort_values(by=['Machine Type','Date'],ascending=True).reset_index(drop=True)
    modified_df = modified_df[['Machine Type','Date','Job Name','RunID','Run Name','Job Type','Total Cells']]
    modified_df.index += 1
